        ...
        return (
            <div className="pagination">
                <div className="pagination-container">
                    {totalPages > 0 && (
                        <a href={this.getLink(page - 1)} disabled={page <= 1} className={cn("pagination-button", {"disabled-link" : page <= 1})}>
                            <i className="fa fa-chevron-left" /> {this.context.t("PAGINATION_PREV")}
                        </a>
                    )}
                    <ul className="pagination-pages">
                        {Array.from(Array(totalPages), (e, i) => {
                            ...
                            return <li key={i} className={cn({"pagination-pages-active" : active})}>
                                <a href={this.getLink(pageNumber)} className={cn({"disabled-link" : active, "no-border": page - 1 === pageNumber})}>
                                    {pageNumber}
                                </a>
                            </li>
                        })}
                    ...
                    {totalPages > 0 && (
                        <a href={this.getLink(page + 1)} className={cn("pagination-button", {"disabled-link" : page >= totalPages})}>
                            {this.context.t("PAGINATION_NEXT")} <i className="fa fa-chevron-right" />
                        </a>
                    )}
                </div>
