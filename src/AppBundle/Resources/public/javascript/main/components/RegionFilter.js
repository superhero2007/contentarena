...
class RegionFilter extends React.Component {
    ...
    selectTerritory(region) {
        ...
        let activeTerritories = this.state.activeTerritories,
    ...
    getSelectedItems = ( allSelected ) => {
        ...
        activeTerritories.forEach( t => {
            selected = selected.concat(territoryItems[t].filter((item) => {
        ...
        activeRegions.forEach( t => {
            selected = selected.concat(regionItems[t].filter((item) => {
                return selected.indexOf(item) < 0;
            }))
        });
