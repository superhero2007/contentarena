...
class ContentListingTable extends Component {
    ...
    getHeader = ( text, tooltip = '', iconClass = '', sortIcon = false ) => {
        ...
        return <span data-tip={tooltip && tooltip}>
            {text && text}
    ...
    getSeasonCell = (seasons) => {
        ...
        if (seasons.length === 1) {
            ...
            let firstSeason = minBy(seasons, (season) => this.getYear(season.startDate));
            let secondSeason = first(seasons.filter(item => item.id !== firstSeason.id));
            ...
            let firstSeason = minBy(seasons, (season) => this.getYear(season.startDate));
            let secondSeason = maxBy(seasons, (season) => this.getYear(season.startDate));
            season = `${this.getYear(firstSeason.startDate)}/${this.getYear(firstSeason.customEndDate)}-${this.getYear(secondSeason.startDate)}/${this.getYear(secondSeason.customEndDate)}`;
        }
    ...
    render() {
        ...
        return <section className="content-listing-table-wrapper">
            <ReactTable
