...
class SalesPackageForm extends React.Component {
  ...
  componentWillUnmount() {
    ...
    setTerritoriesMethod = (territoriesMethod) => {
      this.setState({ territoriesMethod, territories: [] });
          ...
          if (territoriesMethod === this.worldwide) {
            ...
            if (selectedRegion && territories.length <= selectedRegionCountries.length && territories.length >= (selectedRegionCountries.length - 3)) {
              ...
              if (territories.length < selectedRegionCountries.length) {
                ...
                name += excludedFromRegion.map((territory, i) => territory.label).join(", ");
              }
              regionNamed = true;
            } else {
    ...
    getAllCountries = () => Object.values(ContentArena.Data.Countries).map((i, k) => ({ value: i.name, label: i.name }));
...
Installment(s)
                        <input
                          onChange={(e) => { this.setInstallmentValue(e.target.value, i); }}
                    ...
                    <i
                      ...
                      onClick={() => { this.removeInstallment(i); }}
                    />
                    )}
                    { i === list.length - 1 && (
                    <i
