                        ...
                        { this.state.installments.map( (installment, i, list) => {
                            return <div className={"content"}>
                                <div className={"item"} style={{ paddingLeft: 15 }}>
                                    <div className={"title"} >
                                        {i+1} Instalment(s) <input onChange={(e) => {this.setInstallmentValue(e.target.value,i)}} style={{ height: "26px", width: "70px" }} type="number" max={100} value={installment.value}/>{this.context.t("CL_STEP4_INSTALLMENTS_PERCENTAGE")}
                                    </div>
