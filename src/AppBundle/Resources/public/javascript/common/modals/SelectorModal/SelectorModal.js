  ...
  componentWillReceiveProps(nextProps) {
    ...
    getRightHeaderBtn = () => {
      ...
      return (
        <React.Fragment>
          {showNewSport && (
          <button className="ca-btn yellow-border" onClick={() => { this.addNewSport(index); }}>
            {this.context.t("CL_STEP1_SELECTOR_ADD_SPORT")}
          </button>
          )}

          {showNewSeason && (
          <button className="ca-btn yellow-border" onClick={() => { this.addNewSeason(index); }}>
            {this.context.t("CL_STEP1_SELECTOR_ADD_SEASON")}
          </button>
          )}

          {showNewTournament && (
          <button className="ca-btn yellow-border" onClick={() => { this.addNewTournament(index); }}>
