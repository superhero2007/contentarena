...
class PropertyListItem extends React.Component {
	...
	render() {
		const { name, customId, listings = [] } = this.props; // TODO: YU check listings property
		return (
			<section className="property-item-wrapper">
				<div className="property-wrapper d-flex">
					...
					<div className="property-info">
						<a className="ca-title" href={`${ROUTE_PATHS.PROPERTIES}/${customId}`}>{name}</a>
						<div className="bids-wrapper">
							...
							<button className="link-button">{this.context.t("CMS_PROPERTY_VIEW_LISTING_OVERVIEW")}</button>
						</div>
					</div>
				</div>
